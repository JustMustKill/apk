...
/* Ñåêóíäà, ìèíóòà, ÷àñ,  äåíü, ìåñÿö,  ãîä */
...
/* Íîâûå îáðàáîò÷èêè ïðåðûâàíèé */
void interrupt newTimer(...)
{
	delayTime++;
	outp(0x70, 0x0C);
	inp(0x71);
	/* Ïîñûëàåì êîíòðîëëåðàì ïðåðûâàíèé ñèãíàë end of interruption */
	outp(0x20, 0x20);
	outp(0xA0, 0x20);
}
...
void getTime()
{
	...
	for (i = 0; i < 6; i++)
	{
		/* Âûáèðàåì íóæíûé ðåãèñòð */
		outp(0x70, registers[i]);
		/* Ñ÷èòûâàåì çíà÷åíèå èç íóæíîãî ðåãèñòðà â ìàññèâ */
		date[i] = inp(0x71);
	}

	/* Ïåðåâîäèì ñ÷èòàííûå çíà÷åíèå â äåñÿòè÷íóþ ôîðìó */
	int decDate[6];
	for (i = 0; i < 6; i++)
	{
...
{
	/* Çàïðåùàåì ïðåðûâàíèÿ */
	disable();

	/* Óñòàíàâëèâàåì íîâûå îáðàáîò÷èêè ïðåðûâàíèé */
	...
	/* 0xFE = 11111110, áèò 0 â 0, ÷òîáû ðàçðåøèòü ïðåðûâàíèÿ îò ×ÐÂ */

	/* Âûáèðàåì ðåãèñòð B */
	outp(0x70, 0xB);
	outp(0x71, inp(0x71) | 0x40);
	/* 0x40 = 01000000, 6-é áèò ðåãèñòðà B óñòàíàâëèâàåì â 1 äëÿ ïåðèîäè÷åñêîãî ïðåðûâàíèÿ */
...
void setAlarm()
{
	...
	/* Óñòàíàâëèâàåì â ðåãèñòðû áóäèëüíèêà íóæíîå âðåìÿ */
	outp(0x70, 0x05);
	outp(0x71, date[2]);
	...
	/* Âûáèðàåì ðåãèñòð B */
	outp(0x70, 0xB);
	/* Ðàçðåøàåì ïðåðûâàíèå áóäèëüíèêà 5-ì áèòîì */
	outp(0x71, (inp(0x71) | 0x20));

	/* Ïåðåîïðåäåëÿåì ïðåðûâàíèå áóäèëüíèêà */
